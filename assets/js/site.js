(function () {
	...
	const smallScreenQuery = window.matchMedia("(max-width: 736px)");
	...
	async function syncUpdatedDates() {
		...
		const projectDate = await readLastModified("assets/js/projects-data.js?v=20260428e", pageDate);
	...
	function initNeuralField() {
		...
		function makePoints() {
			const count = smallScreenQuery.matches
				? Math.max(10, Math.min(18, Math.round(width / 44)))
				: Math.max(22, Math.min(48, Math.round(width / 34)));
		...
		if (prefersReducedMotion || smallScreenQuery.matches) {
			drawStaticFrame(0);
			return;
		}
